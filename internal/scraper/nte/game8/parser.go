...
var expiryDate = regexp.MustCompile(`(?i)expiry date:\s*(\d{1,2}/\d{1,2}/\d{4})`)
...
func parseHTML(page, sourceURL string, observedAt time.Time) ([]domain.CodeCandidate, error) {
	...
	table.Find("tr").Each(func(_ int, row *goquery.Selection) {
		...
		candidates = append(candidates, domain.CodeCandidate{
			...
			ExpiresAt:  expiration(cells.Eq(codeColumn).Text(), observedAt),
			SourceID:   "nte-game8",
			SourceURL:  sourceURL,
...
func parseRewards(cell *goquery.Selection) []string {
	...
	cell.Find("div.align").Each(func(_ int, reward *goquery.Selection) {
		value := strings.TrimSpace(strings.TrimPrefix(cleanText(reward.Text()), "・"))
		value = normalizeReward(value)
		if value != "" && !seen[value] {
	...
	if len(rewards) == 0 {
		if value := strings.TrimSpace(strings.TrimPrefix(cleanText(cell.Text()), "・")); value != "" {
			rewards = append(rewards, normalizeReward(value))
		}
	}
	return rewards
}

func expiration(value string, now time.Time) *time.Time {
	cleaned := cleanText(value)
	if strings.Contains(strings.ToLower(cleaned), "still active") {
		return nil
	}
	match := expiryDate.FindStringSubmatch(cleaned)
	if len(match) < 2 {
		return nil
	}
	parsed, err := time.ParseInLocation("1/2/2006", match[1], now.Location())
	if err != nil {
		return nil
	}
	parsed = parsed.UTC()
