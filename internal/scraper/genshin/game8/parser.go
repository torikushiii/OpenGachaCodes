...
func parseHTML(page, sourceURL string, observedAt time.Time) ([]domain.CodeCandidate, error) {
	...
	regionColumn := findColumn(headers, "server", "region")
	...
	table.Find("tr").Each(func(_ int, row *goquery.Selection) {
		...
		if !ok || !looksLikeCode(code) || strings.EqualFold(code, "YUANSHEN") {
			return
		}
		if regionColumn >= 0 && cells.Length() > regionColumn && isChina(cells.Eq(regionColumn).Text()) {
			return
		}
